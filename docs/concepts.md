...
As for NDVI, so for EVI, NBR, NDSI, NDWI, and all other direct applications of bands. Pansharpening and band ratios were crammed into a shared conceptual space (of _band computations_ or similar) for decades by practical necessity, but today, with computational pressures relaxed, they do not naturally fit together. More generally, pansharpening is not useful as preprocessing because we no longer need settle for lossy preprocessing. So pansharpening is not fit for algorithmic purposes. Pansharpening is only for human perception. And therefore, pansharpening into anything other than a perceptually uniform space is inefficient.
