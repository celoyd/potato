...
The model is a convolutional neural network that is small (~50k parameters) and of simple design. Potato’s training data is from the Maxar Open Data Program, and like that imagery, it is [licensed](https://github.com/celoyd/potato/tree/main#licensing-data-credit-and-thanks) CC BY-NC. It can be applied (noncommercially) in its present form – see [the quickstart](docs/quickstart.md) – but it is not tested, reviewed, or optimized for real-world use; it’s a research project. It aims to be a good influence on the next generation of applied pansharpening models, not to lead them.

## About pansharpening

_This is an explanation for people first encountering the concept. Intermediate and expert readers will notice several brazen lies in it, and will find [other parts of the documentation](docs) more useful._
...
_The sample images here are of Chungthang, Sikkim, from Maxar 10300100CE8D0400, on 2022-03-07. Like all images shown in this repo except as otherwise credited, they are [CC BY-NC](https://creativecommons.org/licenses/by-nc/4.0/deed.en) from the [Maxar Open Data Program](https://registry.opendata.aws/maxar-open-data/). See [the license and credits](#licensing-and-data-credit) section for more information._

This is called the panchromatic or pan band, and it works like black and white film, or like applying a black and white filter to a photo. A band is a range of the spectrum, and pan-chromatic means all-color – the pan band’s range is all visible light.
...
So we have one image that is sharp, and one that is full-color. We want to merge them into a single combined version that is both sharp _and_ full-color. We can think of this as using the pan image to sharpen the full-color image, so the process is called **pansharpening**.

Like data compression or packet routing, pansharpening is a general task, not any particular algorithm that does the task. There are many different pansharpening methods, and almost as many different ways of defining the details of the task.
...
Here we have two examples of pansharpening these pan and multispectral images of Chungthang.
